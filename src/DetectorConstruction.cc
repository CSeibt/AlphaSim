...
:G4VUserDetectorConstruction(),
 ...
 fTarX(0.), fTarY(0.), fTarZ(0.), fSimWafer(true), fTargetShape("Cylinder"), fGridToAnodeDist(35.*mm)
{
 // fTargetLength      = 1. * cm;
  // fTargetRadius      = 1. * cm;
...
G4VPhysicalVolume* DetectorConstruction::ConstructVolumes()
{
  ...
  // Short rods
  fShortRodPlaceHeight = fAnodeThickness/2. + fRodAnodeGridHolder/2.;
  G4Tubs* SRodShort = new G4Tubs("SRodShort",fRodIRadius,fRodORadius,fRodAnodeGridHolder/2.,0.,twopi);
  fLRodShort = new G4LogicalVolume(SRodShort, fRodMater, "LRodShort", 0, 0, 0);

  // Long Rods
  fLongRodPlaceHeight = fAnodeThickness/2. + fGridToAnodeDist + fGridHolderThickness2 + fRodGridUpHolderTop/2.;
  G4Tubs* SRodLong = new G4Tubs("SRodLong",fRodIRadius,fRodORadius,fRodGridUpHolderTop/2.,0.,twopi);
  fLRodLong = new G4LogicalVolume(SRodLong, fRodMater, "LRodLong", 0, 0, 0);
  for (G4int i = 0; i<3; i++){
    G4double phi = 120.*deg*i;
    G4double x = fRodPlaceRadius*cos(phi);
    G4double y = fRodPlaceRadius*sin(phi);
    G4ThreeVector posRodShortUp = G4ThreeVector(x,y,fShortRodPlaceHeight);
    G4ThreeVector posRodShortDown = G4ThreeVector(x,y,-1*fShortRodPlaceHeight);
    G4ThreeVector posRodLongUp = G4ThreeVector(x,y,fLongRodPlaceHeight);
    G4String nameUp = "PRodShortUp" + std::to_string(i);
    G4String nameDown = "PRodShortDown" + std::to_string(i);
    G4String nameLong = "PRodLong" + std::to_string(i);
    new G4PVPlacement(0, posRodLongUp, fLRodLong, nameLong, fLogicDetector, false, 0);
    new G4PVPlacement(0, posRodShortUp, fLRodShort, nameUp, fLogicDetector, false, 0);
...
void DetectorConstruction::SetSimWafer(G4bool val){
  fSimWafer = val;
}
